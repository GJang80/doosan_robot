...
void OnMonitoringDataExCB(const LPMONITORING_DATA_EX pData) {
  return;
  cout << "# monitoring 1 data " << pData->_tCtrl._tJoint._fActualPos[1]
...
void OnRTMonitoringData(LPRT_OUTPUT_DATA_LIST tData) {
  ...
  if (mCnt == 1000) {
    ...
    printf("joint : %f %f %f %f %f %f\n", tData->actual_joint_position[0],
           tData->actual_joint_position[1], tData->actual_joint_position[2],
           tData->actual_joint_position[3], tData->actual_joint_position[4],
...
int main() {
  ...
  while (bLoop) {
    ...
    switch (ch) {
      ...
      case '4': {
        cout << "Input Version : " << Drfl.get_rt_control_input_version_list()
             << endl;
        cout << "Output Version : " << Drfl.get_rt_control_output_version_list()
             << endl;
      } break;
      case '5': {
        cout << "Input Data List : "
             << Drfl.get_rt_control_input_data_list("v1.0") << endl;
        cout << "Output Data List : "
             << Drfl.get_rt_control_output_data_list("v1.0") << endl;
      }
